...
// All four techniques are presented as equal options. The user chooses.
// No state-based recommendation / pre-selection — per Nadia's feedback,
// state-to-technique mapping is not part of the current agreement and
// should not be inferred on this screen.
...
const techniques: TechniqueOption[] = [
  ...
  },
  {
    ...
    desc: 'Parasympathetic activation',
  },
  {
    id: '90-second-pause',
...
export default function TechniqueSelection() {
  ...
  return (
    <ScreenWrapper showBack backPath="/loop-progress" padBottom={false}>
      ...
      <p className="text-body text-am-text-secondary mt-2">Choose a technique.</p>
      ...
      <div className="flex flex-col gap-3 mt-6 stagger-children">
        {techniques.map((t, i) => (
          ...
          >
            ...
            {t.subtitle && <p className="text-small mt-1">{t.subtitle}</p>}
            <p className="text-small mt-1 text-am-text-secondary">{t.desc}</p>
            {t.extra && <p className="text-micro mt-2">{t.extra}</p>}
          </button>
        ))}
      </div>
